  ...
  if SAY_PERMS != '4':
    ...
    async def say(self, interaction: discord.Interaction, say: str):
      ...
      if interaction.user.id != int(BOT_OWNER_ID) and not interaction.user.guild_permissions.administrator and SAY_PERMS == "2":
        return
      if SAY_PERMS == "3":
        print(f'WARNING: Permissions are very broad for say command and bot will say anything it is told to. Consider '
              f'lowering the permission levels. Currently saying \"{say}\"')
